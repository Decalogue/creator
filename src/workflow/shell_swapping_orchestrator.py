...
class StoryShell:
    ...
    - 题材规则
    """
    shell_type: str  # "urban", "fantasy", "sci-fi", etc.
    background: str = ""
...
class ShellSwappingOrchestrator:
    ...
    def swap_shell(
        ...
        try:
            ...
            # 步骤4：添加调味（由 SeasoningManager 处理）
            # 注意：需要在外部调用 SeasoningManager 添加调味
    ...
    def _add_seasonings(self, story: ShelledStory) -> ShelledStory:
        ...
        Returns:
            添加调味后的故事
        """
        # 由 SeasoningManager 处理
        # 这里暂时跳过，等待 SeasoningManager 实现
        return story
    ...
    def extract_core_from_memory(
        ...
        try:
            ...
            # 过滤出核心记忆（通过 metadata 中的 is_core 标记）
            filtered_memories = []
