...
class NovelTaskWithUniMem(BaseTask):
    ...
    def _format_memories_for_prompt(self, memories):
        ...
        formatted = ["=== 相关记忆 ==="]
        for i, mem in enumerate(memories[:5], 1):  # 只取前5条
...
def run_novel_task_with_unimem(task_data, unimem_enabled=True):
    ...
    # 5. 创建增强的 GlobalInfo（注入记忆）
    # 注意：这需要在 GraphReasoning.start() 之后修改
    # 或者创建一个自定义的 start 方法
    ...
    # 7. 执行推理（Agent 会自动调用工具，包括 UniMem 工具）
    print("\n【步骤 4】执行推理步骤...")
    max_steps = 5
...
# 增强的 Agent 示例：CharacterAgent with UniMem
class CharacterAgentWithMemory:
    ...
    def take_action_with_memory(self, global_info):
        ...
        if memories:
            ...
            # 将记忆添加到 system prompt 或 dialog history
