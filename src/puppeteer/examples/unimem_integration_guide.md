# Puppeteer 与 UniMem 集成指南

## 一、集成方式

### 方式 1：任务级别集成（推荐用于创作任务）
...
def run(runner, evaluator, results_dir, mode, data_limit=None):
    ...
    for task_data in dataset:
        ...
        # Agent 在执行过程中可以：
        # 1. 使用 task["retrieved_memories"] 中的记忆
        # 2. 调用 UniMem 工具存储新记忆
        # 3. 检索特定领域的记忆
