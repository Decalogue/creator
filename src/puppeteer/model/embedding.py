...
class RewardModelTokenRepresentation():
    def __init__(self):
        ...
        # 使用更节省显存的配置
        # 尝试使用 CPU offload 来减少显存使用
        import os
        if os.getenv("USE_CPU_OFFLOAD", "false").lower() == "true":
            # 使用 CPU offload 将部分层移到 CPU
            device_map = "balanced_low_0"  # 将模型分层加载，部分在 CPU
        else:
            device_map = "auto"
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            max_memory={0: "10GiB", "cpu": "30GiB"} if device_map == "auto" else None  # 限制 GPU 显存使用
        )
    ...
    def __call__(self, messages:List):
        with torch.no_grad():
            ...
            tokenized_message = self.tokenizer.apply_chat_template(
                ...
                truncation=True  # 确保截断
            )
            model_log_and_print("tokenized done")
            
            # 获取模型设备（可能是 cuda 或 cpu）
            ...
            with torch.no_grad():  # 确保不使用梯度计算
                response_token_ids = self.model.generate(
                    ...
                    do_sample=False,  # 禁用采样以节省显存
                    use_cache=True,  # 使用 KV cache 加速
                    pad_token_id=self.tokenizer.eos_token_id  # 设置 pad token
                )
