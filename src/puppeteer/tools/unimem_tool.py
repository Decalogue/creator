...
class UniMemRecallTool(Tool):
    ...
    用法：
        recall_result, results = tool.execute(
            query="用户喜欢什么饮料",
            context={"session_id": "session_123", ...},  # 可选
            memory_type="episodic",  # 可选：episodic/semantic/world/experience/observation/opinion
            top_k=10  # 可选，默认10
        )
    """
...
class UniMemReflectTool(Tool):
    ...
    def __init__(self, name):
        super().__init__(
            ...
            execute_function=self.execute,
            timeout_duration=60  # 60秒超时（reflect操作可能较慢）
        )
        self.api_base_url = get_unimem_api_url()
    
    @retry(stop=stop_after_attempt(2), wait=wait_fixed(2))  # reflect操作减少重试次数
    def execute(self, *args, **kwargs):
        """执行 reflect 操作"""
        try:
...
class UniMemFilterTool(Tool):
    ...
    用法：
        filter_result, filtered_context = tool.execute(
            context={"dialog_history": [...], ...},
            filter_criteria="irrelevant",  # 可选：irrelevant/redundant/noise
            preserve_recent=3  # 可选：保留最近N条消息
        )
    """
