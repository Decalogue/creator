...
class CodeInterpreter(Tool):
    ...
    def execute(self, *args, **kwargs):
        try: 
            ...
            ***REMOVED*** raise ValueError(f"Running Error")
    ...
    def robust_kill(self, process):
        ...
        if process.poll() is None:  ***REMOVED*** Check if the process is still running
            if os.name == 'nt':  ***REMOVED*** For Windows
                ...
                time.sleep(1)  ***REMOVED*** Allow some time for graceful termination
                if process.poll() is None:  ***REMOVED*** Force kill if still running
                    os.kill(process.pid, signal.CTRL_BREAK_EVENT)
            else:  ***REMOVED*** For Linux/macOS
                os.killpg(os.getpgid(process.pid), signal.SIGTERM)  ***REMOVED*** Terminate the process group
                time.sleep(1)  ***REMOVED*** Allow some time for graceful termination
                if process.poll() is None:  ***REMOVED*** Force kill the group if still running
    ...
    def run(self, work_path, code_path, file_path):
        ...
        try:
            ...
            ***REMOVED*** Determine the command to run based on the operating system
            ...
            try:
                ***REMOVED*** Wait for process completion with a timeout of 10 seconds
            ...
            ***REMOVED*** If the process is still running after the timeout
            if process.poll() is None:
                self.robust_kill(process)  ***REMOVED*** Ensure the process is terminated
            return_code = process.returncode
