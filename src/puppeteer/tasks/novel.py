...
def format_question(q, idx):
    ...
    return {
        ...
        "Question": question,
        "Answer": q["outline"],
        "Introduction": q["introduction"],  # 添加简介，用于评估
        "id": idx
    }
...
def filter_tool_agents():
    ...
    drop_tool_agent_names = {
        ...
        "BingAgent",       # Bing 搜索
        "WebsiteAgent",    # 网站访问
        "PythonAgent_qwen" # Python 代码执行
    }
    
    # 获取所有已注册的 Agent，包括 unique_agents 和 agents
    ...
    # 更新注册表中的 unique_agents 和 agents
...
def run(runner, evaluator, results_dir, mode, data_limit=None):
    ...
    with open(result_path, "w", encoding="utf-8") as fd:
        for idx, q in enumerate(tqdm(dataset)):
            ...
            # 在每次推理前过滤工具类 Agent
            # 注意：需要在 setup_reasoning 之前调用，因为 AgentGraph 在 setup_reasoning 中初始化
            filter_tool_agents()
            
            # 创建过滤后的 AgentGraph
            graph = AgentGraph()
            # 创建一个自定义的 setup_reasoning
