...
def filter_tool_agents():
    ...
    # 注意：TerminatorAgent 必须保留，因为它用于终止推理
...
def run(runner, evaluator, results_dir, mode, data_limit=None, unimem_enabled=True, personas_path=None):
    ...
    with open(result_path, "w", encoding="utf-8") as fd:
        for idx, q in enumerate(tqdm(dataset, desc="处理任务")):
            ...
            # 过滤工具类 Agent（必须在创建 AgentGraph 之前）
            filter_tool_agents()
            
            # 将带记忆的注册表同步到全局注册表（因为 AgentGraph 使用全局注册表）
            ...
            # 创建 AgentGraph（使用同步后的注册表）
            graph = AgentGraph()
            
            # 使用带记忆的 GraphReasoning
...
if __name__ == "__main__":
    ...
    # 运行任务（启用 UniMem，使用带记忆的 personas）
    run(
        runner=None,
        evaluator=evaluator, 
