...
class ContinuousREINFORCE(LearningPolicy):
    def __init__(self, agent_graph, action_graph, config_path="config/policy.json"):
        ...
        ***REMOVED*** Set parameters from config
        ...
        ***REMOVED*** Initialize state representation and policy network
        ...
        ***REMOVED*** Setup actions and rewards
        ...
        ***REMOVED*** Initialize reward factors from config
    ...
    def logarithmic_cost(self, step):
        ...
        ***REMOVED*** Normalize step to [0,1] range
    ...
    def save_model(self, path=None, tag=None):
        ...
        checkpoint = {
            ...
            'timestamp': timestamp,
            'config': self.config,  ***REMOVED*** Save the config with the model
            'metadata': {
                'tag': tag,
                'version': '1.0',
            }
    ...
    def init_forward(self, global_info):
        ...
        action_probs = self.policy_network(state) ***REMOVED***shape (1,action_dim)
            ...
            for trajectories in self.executed_trajectories[:self.sample_size]:
                ...
                for trajectory in trajectories:
                    if trajectory[-1].get('finalized', False):
                        ...
                        ***REMOVED*** episode_returns.append(sum(returns))
                        ...
                        if task_last_reward[-1] > 0:
                            ...
                            task_acc.append(0)
                        ***REMOVED*** task_acc.append(task_last_reward[-1].cpu().item())
                        ***REMOVED*** episode_lengths.append(len(trajectory))
                        ...
                        for t, R in zip(trajectory, returns):
                            ...
                            if loss.dim() == 0:  ***REMOVED*** scalar loss, convert to shape [1]
                                loss = loss.view(1)
                            elif loss.dim() == 1:  ***REMOVED*** already [1], keep it
                                pass
    ...
    def load_model(self, path, strict=True):
        try:
            ...
            ***REMOVED*** Validate model architecture
            ...
            ***REMOVED*** Load model state
            self.policy_network.load_state_dict(checkpoint['model_state_dict'], strict=strict)
            self.policy_network = self.policy_network.to(self.device)
            
            ***REMOVED*** Load optimizer state if available
            if checkpoint['optimizer_state_dict'] and hasattr(self, 'optimizer'):
                self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                ***REMOVED*** Move optimizer state to correct device
                for state in self.optimizer.state.values():
                    for k, v in state.items():
                        if isinstance(v, torch.Tensor):
                            state[k] = v.to(self.device)
            
            ***REMOVED*** Load config if available
            if 'config' in checkpoint:
                ***REMOVED*** Merge loaded config with current config, prioritizing current config
