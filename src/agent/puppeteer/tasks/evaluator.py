...
class BenchmarkEvaluator:
    ...
    def check_novel(final_ans, true_ans, introduction=None):
        ...
        # 计算综合分数（加权平均）
        # 权重分配原则：
        # - 简介匹配度：最高权重（0.3），因为这是任务的核心要求
        # - 相似度：较高权重（0.25），作为参考指标
        # - 完整性：中等权重（0.2），确保大纲完整
        # - 逻辑一致性：中等权重（0.15），确保情节合理
        # - 结构质量：较低权重（0.1），基础要求
        if introduction:
            # 权重：简介匹配度0.3，相似度0.25，完整性0.2，逻辑性0.15，结构0.1
            ...
            # 如果没有简介，调整权重：相似度0.3，完整性0.25，逻辑性0.25，结构0.2
