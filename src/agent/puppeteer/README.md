***REMOVED*** Multi-Agent Collaboration via Evolving Orchestration
...
4. **Configure the system**
   ```bash
   ***REMOVED*** Edit configurations with your settings
   vim config/global.yaml  ***REMOVED*** Add your API keys
   ```
    - Global Configuration (`config/global.yaml`): configure API access, file paths, and system behavior:
        ```yaml
        ***REMOVED*** API Configuration
        logging:
            level: INFO               ***REMOVED*** Logging level, options: DEBUG, INFO, WARNING, ERROR
            logpath: ./logs           ***REMOVED*** Folder path to store log files

        ***REMOVED*** Path to the folder containing model weights of the Puppeteer base model
        ***REMOVED*** (downloaded in step 3, or directly loading)
        model_weight_path: nvidia/Llama-3.1-Nemotron-70B-Reward

        api_keys:
            openai_api_key: ""        ***REMOVED*** Your OpenAI API key
            openai_base_url: "https://api.openai.com/v1/"  ***REMOVED*** OpenAI base URL
            bing_api_key: ""          ***REMOVED*** Bing API key for web search (optional)

        ***REMOVED*** System retry settings
        max_retry_times: 10           ***REMOVED*** Maximum number of times to retry API calls
        max_json_reformat_turns: 10   ***REMOVED*** Maximum retries for JSON parsing/reformatting

        ***REMOVED*** Enable external tools (like web search, file read, etc.)
        external_tools_enabled: True

        ***REMOVED*** File paths that agents may need
        file_path:
            root_file_path: ./data    ***REMOVED*** Root folder containing all necessary files for agents

        ***REMOVED*** Graph exploration parameters for multi-agent reasoning
        graph:
            max_parallel_paths: 4     ***REMOVED*** Maximum number of parallel paths to explore (recommended 2-6)
            max_step_num: 5           ***REMOVED*** Maximum number of steps (nodes) in each path (recommended 4-6)
        ```
...
5. **Quick start with the predefined settings**
    ...
    Example:
    ```bash
    ***REMOVED*** Run MMLU-Pro validation set with a data limit of 10
...
***REMOVED******REMOVED*** Agents
***REMOVED******REMOVED******REMOVED*** 🔎 Agent Categories
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 2. Adding New Actions  
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. Supporting New Base Models  
...
***REMOVED******REMOVED*** 🎭 Puppeteer Training  
...
***REMOVED******REMOVED******REMOVED*** 🔹 Agent Scale Control  
- `max_num_agents`: `3`  
...
***REMOVED******REMOVED******REMOVED*** 🔹 Reward Configuration  
- `gamma`: `0.99`  
...
***REMOVED******REMOVED******REMOVED*** 🔹 Cost Control  
- `scale`: `0.1`  
...
***REMOVED******REMOVED******REMOVED*** 🔹 Training Paradigm  
