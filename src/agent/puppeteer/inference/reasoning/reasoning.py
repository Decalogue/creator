...
class GraphReasoning:
    ...
    def start(self, save_data):
        ...
        ***REMOVED*** -1 is the default path id for intialization
    ...
    def step(self):
        ...
        for reasoning_path in self.reasoning_paths[:self.max_parallel_paths]:
            ***REMOVED*** Deal with the case where the reasoning path is not finalizing and not spliting
        ...
        for reasoning_path in self.reasoning_paths[:self.max_parallel_paths]: 
            ***REMOVED*** Deal with the case where the reasoning path is spliting
            ...
            ***REMOVED*** Deal with the case where the reasoning path is finalizing
    ...
    def aggregate_answers(self, global_info, answers:list, query_func=None) -> str:
        ***REMOVED*** only choose the last result without any format or extract
        ...
        ***REMOVED*** only choose the last result without any format or extract
        ...
        elif self.task.get("type") == "Novel":
            answer_prompt = "\n".join(prompt["novel_aggregation"]).format(str(["{}\n".format(answer) for answer in answers]))
        else: 
    ...
    def majority_vote(self, answers: List) -> str:
        ...
        for answer in answers:
            answer = str(answer).strip()  ***REMOVED*** Convert to string and remove whitespace
            answer_counts[answer] = answer_counts.get(answer, 0) + 1
        
        if not answer_counts:
            return ""  ***REMOVED*** Return empty string if no answers
    ...
    def finalize(self):
        ...
        for idx, reasoning_path in enumerate(self.reasoning_paths):
            ...
            if self.task.get("type") == "MMLU-Pro":
                ...
                reward, metrics = BenchmarkEvaluator.check_novel(
                    aggregated_answer, 
                    self.task.get("Answer"),
                    introduction=self.task.get("Introduction")  ***REMOVED*** 传入小说简介用于评估
                )
                transition = {
                    'state': reasoning_path.global_info.workflow.state,
                    'reward': reward,
        ...
        if len(self.answers) == 1 or self.task.get("type") == "SRDD" or self.task.get("type") == "CW" or self.task.get("type") == "Novel":
