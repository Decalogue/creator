***REMOVED***!/bin/bash
***REMOVED*** 监控重写机制优化测试进度（需在 src 下执行，或从项目根执行时路径已自动切到 src）

TEST_LOG="/tmp/test_rewrite_optimization.log"
OUTPUT_DIR="task/novel/outputs/重写机制优化测试"
...
if pgrep -f "test_quality_optimizations.py.*重写机制优化测试" > /dev/null; then
    echo "✅ 测试正在运行中..."
else
    echo "⚠️  测试未运行或已完成"
fi
