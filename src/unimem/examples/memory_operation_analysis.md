...
# 自动检测和降级
config = {
    "graph": {"backend": "memory"},  # 内存模式，不需要 Neo4j
    "storage": {"backend": "memory"},  # 内存模式，不需要 Redis
    "vector": {"backend": "memory"}  # 内存模式，不需要 Qdrant
}
```
...
```python
# 阶段1: 仅基本功能（无需 UniMem）
adapter = VideoAdapter()  # 独立模式

# 阶段2: 启用本地存储
adapter.enable_local_memory()  # 文件存储

# 阶段3: 启用完整 UniMem
adapter.enable_unimem(config)  # 完整功能
```
