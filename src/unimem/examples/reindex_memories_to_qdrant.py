...
def reindex_all_memories():
    ...
    # 直接检查Qdrant连接（使用HTTP API避免客户端版本问题）
    try:
        import requests
    ...
    for i, row in enumerate(results, 1):
        try:
            ...
            # 直接使用network_adapter的方法添加向量
            # 因为embedding_model可用，即使qdrant_client可能未初始化，我们也可以手动添加
            try:
                # 生成embedding
                ...
                # 处理ID - Qdrant需要字符串或整数ID
                # 使用memory.id的hash作为整数ID，或者使用字符串
                try:
                    # 尝试使用UUID的int表示（取前64位）
                    uuid_obj = uuid_lib.UUID(memory.id)
                    # 使用字符串ID（Qdrant支持字符串ID）
