...
class NovelAnalyzer:
    ...
    def __init__(self, window_size: int = 512, overlap: int = 64):
        ...
        # 初始化 UniMem 和 AtomLinkAdapter
        config = {
            "graph": {
                "backend": "networkx",  # 使用轻量级 networkx，不需要 Neo4j
            },
            "vector": {
                "backend": "qdrant",  # 使用 Qdrant 作为向量数据库
            },
            "network": {
        ...
        self.memory = UniMem(
            config=config,
            storage_backend="redis",  # 默认存储后端
            graph_backend="networkx",  # 使用轻量级 networkx
            vector_backend="qdrant",  # 使用 Qdrant
        )
        # 直接访问 network_adapter（即 AtomLinkAdapter）
    ...
    def analyze_segment(self, segment: str, segment_id: int, context: Context) -> Memory:
        ...
        Returns:
            分析后的记忆对象
        """
        # 使用 AtomLinkAdapter 构建原子笔记
        # 这会自动使用 LLM 分析内容，提取 keywords, context, tags
        # 并生成结构化的 Memory 对象
        timestamp = datetime.now()
        
        # 提取实体（这里可以后续扩展，从片段中提取角色、地点等实体）
        entities = []  # 暂时为空，后续可以添加实体提取逻辑
        
        # 使用 construct_atomic_note 构建原子笔记
        # 这会调用 LLM 分析内容，生成结构化的摘要和元数据
        # generate_summary=True 表示生成摘要性内容，而不是直接使用原始片段
        memory = self.adapter.construct_atomic_note(
            content=segment,
            timestamp=timestamp,
            entities=entities,
            generate_summary=True  # 生成摘要，而不是直接使用原始文本
        )
        ...
        for mem in memories:
            ...
            if mem.links:
                ...
                for linked_id in mem.links:
                    if linked_id in memory_map:  # 确保链接的记忆存在
                        edge = {
                            "source": mem.id,
                            "target": linked_id,
