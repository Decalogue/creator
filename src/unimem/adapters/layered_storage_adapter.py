...
class LayeredStorageAdapter(BaseAdapter):
    ...
    def add_to_foa(self, memory: Memory) -> bool:
        ...
        try:
            with self._foa_lock:
                ...
                # 如果超过 token 预算或记忆数量限制，移除最旧的记忆（FIFO）
    ...
    def search_foa(self, query: str, top_k: int = 10) -> List[Memory]:
        ...
        try:
            with self._foa_lock:
                ...
                # 使用内存存储（默认或降级）
                return self.foa_storage[-top_k:].copy()  # 返回副本，避免外部修改
        except Exception as e:
    ...
    def cleanup_old_memories(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            # 清理 DA 中的旧记忆（FoA 通过 token 预算自动管理，LTM 通常不清理）
            cleaned_da = 0
            with self._da_lock:
                da_to_remove = [
