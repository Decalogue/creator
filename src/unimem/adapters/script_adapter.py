...
from .atom_link_adapter import AtomLinkAdapter  ***REMOVED*** 复用基础架构
...
{content[:2000]}"""
        ...
        try:
            messages = [
                {"role": "system", "content": "你是一个专业的短剧剧本分析助手，擅长提取剧本的结构化信息和创作维度。请始终以有效的 JSON 格式返回结果。"},
    ...
    def structure_script_hierarchy(self, episodes: List[str], level: str = "episode_outline") -> Dict[str, Any]:
        ...
        Args:
            episodes: 分集内容列表
            level: 结构化层级 ("episode_outline" | "shot_script" | "full_script")
...
分集内容：
{chr(10).join([f"第{i+1}集: {ep[:500]}" for i, ep in enumerate(episodes[:10])])}
...
分集大纲：
{chr(10).join([f"大纲{i+1}: {ep[:400]}" for i, ep in enumerate(episodes[:5])])}
...
{{
    "shot_scripts": [
        ...
        ...
            ...
            else:  ***REMOVED*** level == "full_script"
                ***REMOVED*** 分镜脚本 -> 完整剧本
...
分镜脚本：
{chr(10).join([f"{ep[:600]}" for ep in episodes[:3]])}
...
- 镜头提示"""
            ...
            messages = [
                {"role": "system", "content": "你是一个专业的短剧剧本创作助手，擅长进行多层级结构化分析。请始终以有效的 JSON 格式返回结果（除完整剧本外）。"},
                {"role": "user", "content": prompt}
            ]
            ...
            ***REMOVED*** 解析结果
            if level == "full_script":
                ***REMOVED*** 完整剧本直接返回文本
                return {"script": response_text.strip()}
            else:
                result = self._parse_json_response(response_text)
                if result:
                    return result
            ...
            logger.error(f"Error structuring script hierarchy: {e}")
            return {}
    
    def generate_script_from_outline(self, outline: str, target_level: str = "full_script", context_memories: List[Memory] = None) -> str:
        """
        ...
        Args:
            outline: 故事大纲
            target_level: 目标层级 ("episode_outline" | "shot_script" | "full_script")
...
{{
    "shot_scripts": [
        ...
        ...
            ...
            else:  ***REMOVED*** target_level == "full_script"
                prompt = f"""请根据以下分镜脚本生成完整的短剧剧本。
...
..."""
            ...
            messages = [
                {"role": "system", "content": "你是一个专业的短剧剧本创作助手，擅长根据结构化内容生成短剧剧本。请保持风格一致性和节奏控制。"},
                {"role": "user", "content": prompt}
            ]
            ...
            ***REMOVED*** 清理响应文本
            if target_level in ["episode_outline", "shot_script"]:
                ...
                if result:
                    if target_level == "episode_outline":
                        return json.dumps(result.get("episode_outlines", []), ensure_ascii=False, indent=2)
                    else:
                        return json.dumps(result.get("shot_scripts", []), ensure_ascii=False, indent=2)
            
            ***REMOVED*** 对于完整剧本，直接返回文本
            return response_text.strip()
        except Exception as e:
            logger.error(f"Error generating script from outline: {e}")
            return ""
    
    def construct_script_note(self, content: str, timestamp: datetime, entities: List[Entity], generate_summary: bool = True) -> Memory:
        """
        构建短剧剧本原子笔记
        ...
        ***REMOVED*** 存储到内存中
        ...
        logger.debug(f"Constructed script note: {memory_id} with {len(memory.keywords)} keywords, {len(memory.tags)} tags")
        return memory
...
请直接返回摘要内容，不要包含其他格式："""
        ...
        try:
            messages = [
                {"role": "system", "content": "你是一个专业的短剧剧本分析助手，擅长提取剧本的核心信息和关键情节。"},
                {"role": "user", "content": prompt}
            ]
    ...
    def retrieve_script_with_reward(self, query: str, original_scripts: List[str], top_k: int = 10) -> List[Dict[str, Any]]:
        """
        根据结构化结果检索原剧本，并计算奖励
        ...
        try:
            ...
            for idx, memory in enumerate(similar_memories[:top_k]):
                ***REMOVED*** 尝试匹配到原始剧本
                ...
                for orig_idx, orig_script in enumerate(original_scripts):
                    if memory.content[:100] in orig_script or orig_script[:100] in memory.content:
            ...
            ***REMOVED*** 按奖励排序
            ...
            logger.error(f"Error retrieving script with reward: {e}")
            return []
    
    def optimize_script_prompt(self, input_text: str, execution_result: str, current_prompt: str = None) -> Dict[str, Any]:
        """
...
{{
    ...
    "optimized_context": {{
        ...
        "script_specific": {{
            ...
            messages = [
                {"role": "system", "content": "你是一个专业的短剧剧本创作优化助手，擅长分析执行结果并优化 prompt 和上下文结构。请始终以有效的 JSON 格式返回结果。"},
                {"role": "user", "content": prompt}
            ]
            ...
            logger.error(f"Error optimizing script prompt: {e}")
            return {"optimized_prompt": current_prompt or "", "optimized_context": {}, "analysis": {}}
