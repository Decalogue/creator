...
## UniMem 当前实现 vs Context Graph 理念
...
#### 1.1 在Memory中添加决策痕迹字段
```python
class Memory:
    # 现有字段...
    
    # 新增字段
    decision_trace: Optional[Dict[str, Any]] = None  # 决策痕迹
    reasoning: Optional[str] = None  # 决策理由（"为什么"）
    inputs: Optional[List[str]] = None  # 输入数据/上下文
    rules_applied: Optional[List[str]] = None  # 应用的规则
    exceptions: Optional[List[str]] = None  # 异常处理
    approvals: Optional[List[str]] = None  # 审批流程
```

#### 1.2 在RETAIN时捕获完整决策上下文
```python
...
def reflect(self, memories: List[Memory], task: Task, ...) -> Dict[str, Any]:
    ...
    for pattern in reasoning_patterns:
        experience = Memory(
            ...
            reasoning=pattern["why"],  # 新增：为什么
            decision_trace=pattern["trace"],  # 新增：决策痕迹
            memory_type=MemoryType.EXPERIENCE
        )
```

#### 2.2 在Neo4j中存储reasoning字段
```python
# neo4j.py
def create_memory(memory: Memory) -> bool:
    node = Node(
        "Memory",
        # 现有字段...
        reasoning=memory.reasoning or "",  # 新增
