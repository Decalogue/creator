...
### 3.2 存储管理层（基于 LayeredStorageAdapter + MemoryTypeAdapter）
...
### 3.3 网络组织层（基于 GraphAdapter + NetworkLinkAdapter）
...
```
unimem/
├── __init__.py              # 包初始化文件，暴露核心类
├── core.py                   # UniMem 核心逻辑实现
├── types.py                  # 自定义数据类型定义
├── config.py                 # 系统配置管理
├── adapters/                 # 功能适配器层
│   ├── __init__.py
...
│   ├── network_link_adapter.py  # 网络链接适配器
...
├── storage/                  # 存储管理模块
│   ├── __init__.py
│   └── storage_manager.py    # 存储管理器（使用 LayeredStorageAdapter）
├── network/                  # 网络组织模块
│   ├── __init__.py
│   └── network_manager.py    # 网络管理器（使用 GraphAdapter + NetworkLinkAdapter）
├── retrieval/               # 检索引擎模块
│   ├── __init__.py
│   └── retrieval_engine.py  # 检索引擎（使用各适配器）
├── update/                  # 更新机制模块
│   ├── __init__.py
