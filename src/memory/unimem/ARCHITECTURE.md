...
- [执行摘要](#执行摘要)
- [一、UniMem 2.0 架构设计](#一unimem-20-架构设计)
- [二、六大架构在 UniMem 中的角色](#二六大架构在-unimem-20-中的角色)
- [三、核心实现详解](#三核心实现详解)
- [四、实施路径](#四实施路径)
- [五、技术栈](#五技术栈)
- [六、下一步行动](#六下一步行动)
...
# 初始化系统
memory = UniMem(
    storage_backend="redis",  # FoA/DA 使用 Redis
    graph_backend="neo4j",    # 图结构使用 Neo4j
    vector_backend="qdrant"   # 向量存储使用 Qdrant
)
...
```python
# 三操作范式
memory.retain(experience)  # 存储记忆
results = memory.recall(query)  # 检索记忆
memory.reflect(memories, task)  # 更新记忆
```
...
class UniMem:
    ...
    def __init__(self):
        ...
        # 存储层
        self.storage = CogMemStorage()  # FoA/DA/LTM
        self.memory_types = MemMachineTypes()  # 多类型记忆
        
        # 网络层
        self.graph_structure = LightRAGGraphStructure()  # 底层图结构
        self.memory_network = AMemNetwork()  # 上层原子笔记网络
    ...
    def recall(self, query: str, memory_type: str = None):
        ...
        # 3. 多维检索引擎
        # LightRAG 双层检索
        entity_results = self.retrieval_engine.entity_retrieval(query)  # 低级别
        abstract_results = self.retrieval_engine.abstract_retrieval(query)  # 高级别
...
class RippleEffectUpdater:
    ...
    def trigger_ripple(self, center: MemoryNote, entities: List[Entity],
        ...
        self.sleep_updater.add_to_queue(wave3)  # 延迟到睡眠时间更新
...
class Entity:
    """实体表示"""
    name: str  # 实体名称
    entity_type: str  # 实体类型（PERSON, ORGANIZATION, CONCEPT等）
    description: str  # 实体描述（由 LLM 生成）
    retrieval_key: str  # 检索键（用于语义匹配）
    retrieval_value: str  # 检索值（详细描述）
    original_chunks_id: List[str]  # 原始文本块ID
    neighbors: List[str]  # 邻居实体ID
...
class Relation:
    ...
    retrieval_key: str  # 抽象检索键（由 LLM 生成）
    retrieval_value: str  # 检索值
    original_chunks_id: List[str]  # 原始文本块ID
```
...
#### 1. **四大逻辑网络 (Four Logical Networks)**
...
class CARAReflect:
    ...
    def adjust_confidence(self, opinion: Opinion, 
        ...
        - 实现信念的动态演化
        """
        # 强化机制：新证据支持现有观点时增加置信度
        # 削弱机制：新证据与现有观点矛盾时降低置信度
        pass
```
...
class NarrativeFact:
    ...
    叙事性事实单元
    """
    participants: List[str]  # u: 参与者
    background: str  # b: 背景
    topic: str  # t: 主题
    verb: str  # v: 动词/动作
    start_timestamp: datetime  # τ_s: 开始时间戳
    end_timestamp: datetime  # t_e: 结束时间戳
    mention_timestamp: datetime  # t_m: 提及时间戳
    fact_type: str  # l: 事实类型
    confidence: float  # c: 置信度
    metadata: Dict[str, Any]  # x: 其他元数据
...
class Opinion:
    ...
    evidence: List[str]  # 支持证据（记忆ID列表）
    timestamp: datetime  # 形成/更新时间
...
class FocusOfAttention:
    ...
    def __init__(self):
        ...
        self.max_tokens = 100  # 极小的 token 预算
...
#### 4. **双代理协作 (Dual Agent Collaboration)**
...
class MemoryNote:
    ...
    c_i: str  # 原始交互内容
    t_i: datetime  # 交互时间戳
    K_i: List[str]  # 关键词（LLM 生成，捕获核心概念）
    G_i: List[str]  # 分类标签（LLM 生成）
    X_i: str  # 上下文描述（LLM 生成，提供丰富的语义理解）
    e_i: np.ndarray  # 密集向量表示（用于相似度匹配和检索）
    L_i: Set[str]  # 链接的记忆ID集合（语义相关的记忆）
...
class NoteConstruction:
    ...
    def __init__(self, llm: LLM, text_encoder: TextEncoder):
        ...
        self.prompt_template = P_s1  # 笔记构建提示模板
...
class LinkGeneration:
    ...
    def __init__(self, llm: LLM, similarity_threshold: float = 0.7):
        ...
        self.prompt_template = P_s2  # 链接判断提示模板
...
class MemoryEvolution:
    ...
    def __init__(self, llm: LLM):
        self.llm = llm
        self.prompt_template = P_s3  # 记忆演化提示模板
...
memory = {
    ...
    "type": "user_profile"  # 记忆类型
}

# ❌ 不好的实践
memory = {
    "text": "用户喜欢喝咖啡"  # 缺少上下文和元数据
}
```
...
# ❌ 不好的实践：直接查 LTM
results = ltm.search(query)  # 忽略了 FoA 和 DA 的缓存优势
```
...
# ❌ 不好的实践：全部实时更新
for memory in memories:
    update_immediately(memory)  # 可能导致性能问题
```
