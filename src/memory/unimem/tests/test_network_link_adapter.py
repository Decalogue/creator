...
from unimem.adapters.network_link_adapter import NetworkLinkAdapter
from unimem.types import Memory, Entity, MemoryType, MemoryLayer


class TestNetworkLinkAdapter(unittest.TestCase):
    """NetworkLinkAdapter 测试类"""
    ...
    def setUp(self):
        ...
        ***REMOVED*** 创建适配器实例（使用真实模型，Qdrant 连接失败时使用 Mock）
        self.adapter = NetworkLinkAdapter(config=self.config)
    ...
    def test_search_similar_memories(self):
        ...
        ***REMOVED*** Mock Qdrant 搜索结果
        ...
        self.mock_qdrant_client.search.return_value = [mock_result1, mock_result2]
...
class TestNetworkLinkAdapterIntegration(unittest.TestCase):
    """NetworkLinkAdapter 集成测试（需要实际服务）"""
    ...
    def test_full_workflow(self):
        ...
        ***REMOVED*** 检查 Qdrant 是否可用
        ...
        adapter = NetworkLinkAdapter(config=config)
        adapter.initialize()
