    ...
    def _get_direct_related(
        ...
        if self.graph_adapter:
            for entity in entities:
                # TODO: 实现获取实体邻居的方法
                # 目前通过 get_entities_for_memory 间接获取
                # neighbors = self.graph_adapter.get_neighbors(entity.id)
                pass
        
        # 通过网络链接适配器找到相关记忆（参考 A-Mem）
        if self.network_link_adapter:
            for link_id in links:
                # TODO: 实现从链接ID获取记忆的方法
                # linked_memory = self.network_link_adapter.get_memory(link_id)
                pass
