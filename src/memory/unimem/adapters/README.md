...
### 2. LayeredStorageAdapter（分层存储适配器）
...
def recall(self, query, context):
    # 并行检索
    entity_results = self.graph_adapter.entity_retrieval(query)  # 参考 LightRAG
    semantic_results = self.network_adapter.semantic_retrieval(query)  # 参考 A-Mem
    temporal_results = self.retrieval_adapter.temporal_retrieval(query)  # 参考 CogMem
...
# 完整的 retain 流程
retain() 
  -> extract_entities()      # 图结构适配器（参考 LightRAG）
  -> construct_note()        # 网络链接适配器（参考 A-Mem）
  -> classify()              # 记忆分类适配器（参考 MemMachine）
  -> add_to_foa/da/ltm()     # 分层存储适配器（参考 CogMem）
  -> generate_links()        # 网络链接适配器（参考 A-Mem）
  -> trigger_ripple()        # 更新适配器（参考 LightMem + A-Mem）
```
...
```python
# 参考 LightRAG 的实现思路
from lightrag import LightRAG  # 导入参考架构

class GraphAdapterImpl(GraphAdapter):
    def _do_initialize(self):
        # 参考 LightRAG 的初始化思路
        self.lightrag = LightRAG(...)  # 但接口按照 UniMem 的需求定义
    
    def extract_entities_relations(self, text):
        # 调用 LightRAG 的方法，但转换为 UniMem 的类型
