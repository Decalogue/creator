...
class OperationAdapter(BaseAdapter):
    ...
    def retain(self, experience: Experience, context: Context) -> Memory:
        ...
        但接口按照 UniMem 的需求定义
        """
        # 参考 HindSight 的实现思路，但返回 UniMem 的 Memory 类型
        logger.debug(f"RETAIN: Storing experience: {experience.content[:50]}...")
        
        # TODO: 集成 HindSight 的 retain 逻辑
        # 这里应该调用 HindSight 的 put 方法，但转换为 UniMem 的 Memory 格式
    ...
    def recall(self, query: str, context: Context, top_k: int = 10) -> List[Memory]:
        ...
        # TODO: 集成 HindSight 的 recall 逻辑
        # 这里应该调用 HindSight 的 search 方法，但转换为 UniMem 的 Memory 格式
    ...
    def reflect(self, memories: List[Memory], task: Task) -> List[Memory]:
        ...
        # TODO: 集成 HindSight 的 reflect 逻辑
        # 这里应该调用 HindSight 的 think 方法，但转换为 UniMem 的 Memory 格式
