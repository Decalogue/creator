...
- **语义网格**：提供人物、关系、实体结构，供 create_chapter 参考以保持角色与世界观一致
- **云端长期记忆**：提供跨章叙事细节（具体数字、技术术语、一次性事件），与 mesh 分工明确
- **长程召回**：续写章节≥21 时，recall 会额外启用长程细节查询，以获取早期章节的叙事细节
- **关键细节点 retain**：retain_chapter 除摘要外，会提取并写入含数字/编号/技术术语的句子，便于长程召回精确回指
