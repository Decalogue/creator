...
def chat_with_skills(messages: List[Dict[str, str]], stream: bool = False) -> Any:
    ...
    Returns:
        对话结果（字符串或生成器）
    """
    # 1. 获取所有可用的函数定义（OpenAI Function Calling 格式）
    functions = default_registry.get_all_functions()
    
    # 2. 调用 LLM，传入函数定义
    if stream:
        # 流式调用（简化版，实际需要处理 tool_calls）
...
def example_api_integration():
    ...
    api_code = '''
# 在 api_fastapi.py 或 api_flask.py 中使用
...
async def chat(request: ChatRequest):
    messages = request.messages[-10:]  # 最近10轮对话
