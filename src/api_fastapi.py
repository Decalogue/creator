...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        messages = request.messages[-10:]  # 选最近的10轮对话
        # 转换为字典格式，因为 chat.py 中的函数期望字典列表
        messages_dict = [{'role': msg.role, 'content': msg.content} for msg in messages]
        stream = request.stream
        model = request.model or 'DeepSeek-v3-2'  # 默认使用 DeepSeek-v3-2
        ...
        if model == 'DeepSeek-v3-2' or model == 'deepseek-v3-2':
            ...
            # TODO: 实现 Gemini 3 Pro/Flash 的调用
            # 目前暂时使用 deepseek-v3-2 作为 fallback
            if stream:  # 流式数据请求
                def stream_data():
                    for t in ark_deepseek_v3_2_stream(messages_dict):
                        yield t
...
def start(host='0.0.0.0', port=5200, ssl_context=None):
    if ssl_context is not None:  # https
        uvicorn.run(
            app,
            host=host,
            port=port,
