...
***REMOVED*** 定义简单的 NovelChapter（不依赖 react_novel_creator）
@dataclass
class NovelChapter:
    """小说章节（测试用）"""
    chapter_number: int
...
def test_chapter_processing():
    ...
    try:
        ...
        ***REMOVED*** 模拟 Novel Creator 的章节处理逻辑
        mesh = SemanticMeshMemory()
        bus = PubSubMemoryBus()
...
天空中飘着蓝色的云，这是一个充满魔法的世界。''',
        ...
        ***REMOVED*** 处理章节（模拟 _process_chapter_with_creative_context 的逻辑）
        ***REMOVED*** 1. 创建章节实体
