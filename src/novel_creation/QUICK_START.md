...
creator = ReactNovelCreator(
    novel_title="我的小说",
    enable_enhanced_extraction=True,  # 增强实体提取
    enable_quality_check=True  # 质量检查
)
...
for chapter_info in plan['chapter_outline']:
    ...
    if quality.get('total_issues', 0) > 0:
        ...
        for issue in quality['issues'][:3]:  # 显示前3个问题
...
orchestrator = ReActOrchestrator(
    ...
    enable_unimem=True  # 可选：需要 UniMem
)
...
chapter = hybrid.create_chapter(
    chapter_number=1,
    chapter_title="第一章",
    chapter_summary="主角进行激烈的战斗",  # 包含"战斗"，会选择 Puppeteer（如果可用）
    target_words=3000
)
```
...
creator = ReactNovelCreator(
    ...
    enable_unimem=True,  # 需要 UniMem 服务
...
creator = ReactNovelCreator(
    ...
    enable_creative_context=True,
    enable_enhanced_extraction=False,  # 使用基础提取
    enable_unimem=False,
...
if quality:
    ...
    # 查看问题详情
    for issue in quality.get('issues', [])[:5]:  # 前5个问题
...
creator = ReactNovelCreator(
    novel_title="我的小说",
    enable_enhanced_extraction=False  # 节省 Token
)
```
...
creator = ReactNovelCreator(
    novel_title="我的小说",
    enable_context_offloading=True  # 自动管理上下文长度
)
```
