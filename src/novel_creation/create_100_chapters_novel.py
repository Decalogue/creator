...
# 设置环境变量：只使用 kimi_k2 进行实体提取，不使用 gemini_3_flash
os.environ["USE_SINGLE_MODEL_EXTRACTION"] = "1"

# 添加项目路径
project_root = Path(__file__).parent.parent.parent
sys.path.insert(0, str(project_root))
sys.path.insert(0, str(Path(__file__).parent.parent))  # 添加src目录到路径
...
def create_100_chapters_novel():
    ...
    # 小说基本信息（基于30章测试的主题，但创作全新故事）
    # 小说名：针对番茄小说网受众，选择更有冲击力和画面感的标题
    # 候选标题分析：
    # 1. 《完美之墙》- 来自30章测试第1章标题，有画面感，暗示完美背后的危机，最有吸引力
    # 2. 《逻辑死锁》- 直接点题，有科技感，暗示困境
    # 3. 《优化陷阱》- 暗示完美优化的危险
    # 4. 《验证者》- 比《验证》更有角色感
    # 5. 《验证》- 简洁但可能对番茄受众吸引力不够
    # 最终选择：《完美之墙》- 最有画面感和吸引力，符合番茄小说受众喜好
    novel_title = "完美之墙"
    genre = "科幻"
    # 主题：结合番茄小说内容创作规范，确保内容健康、积极，避免低俗、敏感内容
    # 强调：AI优化、智能学习、人性探索、自由意志、悬念结局
    # 避免：低俗色情、未成年人负面导向、血腥暴力、敏感政治话题
    # 语言要求：通俗易懂，避免复杂科学术语，保证绝大多数人都能看得爽
    ...
    creator = ReactNovelCreator(
        ...
        llm_client=kimi_k2  # 使用Kimi K2作为主模型
    )
    
    # 创作小说（使用渐进式大纲，因为章节数>=50）
    try:
        result = creator.create_novel(
            ...
            use_progressive=True  # 强制使用渐进式大纲
        )
