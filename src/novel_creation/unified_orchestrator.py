...
class ReActOrchestrator(NovelOrchestrator):
    ...
    def store_memory(self, chapter: NovelChapter):
        """存储记忆"""
        # 语义网格已自动存储（通过 _process_chapter_with_creative_context）
...
class PuppeteerOrchestrator(NovelOrchestrator):
    ...
    def __init__(
        ...
        """
        # TODO: 实现 Puppeteer 集成
        # from puppeteer import PuppeteerSystem
        # self.puppeteer = PuppeteerSystem(config=puppeteer_config)
...
class HybridOrchestrator(NovelOrchestrator):
    ...
    def _select_orchestrator(self, chapter_summary: str) -> str:
        ...
        Returns:
            'react' 或 'puppeteer'
        """
        # 简单规则：包含"对话"、"心理"等关键词时使用 ReAct
        # 包含"行动"、"战斗"等关键词时使用 Puppeteer（如果可用）
