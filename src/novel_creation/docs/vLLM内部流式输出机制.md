***REMOVED*** vLLM 内部流式输出机制

vLLM 的流式输出在 **引擎层** 通过 **asyncio.Queue + 异步生成器** 实现，与 HTTP/SSE 解耦。本文梳理引擎内部如何 `yield` 增量结果。

---

***REMOVED******REMOVED*** 0. 引擎与模型端：谁封装谁？模型端输出是什么？

**是的，引擎封装了模型端。** 引擎负责调度、请求管理、把模型输出整理成 `RequestOutput`；**模型端**（Worker / Model Runner）负责真正的 forward、采样，跑在单独进程或 Ray worker 里。

***REMOVED******REMOVED******REMOVED*** 0.1 模型端的输出链路（Worker / Model Runner）

模型端每 **一步 decoding** 的产出大致如下：

```
model(input_ids, positions, ...)
    → hidden_states   ***REMOVED*** [batch, seq, hidden_dim]，Decoder 最后一层隐状态

model.compute_logits(hidden_states[logits_indices])
    → logits         ***REMOVED*** [num_tokens, vocab_size]，每个位置对词表未归一化分数

Sampler(logits, sampling_metadata)   ***REMOVED*** softmax + temperature / top_p / top_k 等
    → SamplerOutput:
         - sampled_token_ids: Tensor  ***REMOVED*** 本步采样的 token id
         - logprobs_tensors: 可选     ***REMOVED*** 各候选的 log 概率

封装并返回
    → ModelRunnerOutput:
         - req_ids, req_id_to_index
         - sampled_token_ids: list[list[int]]  ***REMOVED*** 按请求的本步新 token ids
...
***REMOVED******REMOVED******REMOVED*** 0.2 引擎侧：从模型端输出到 RequestOutput

引擎拿到 `ModelRunnerOutput` 后，由 **Scheduler** 和 **OutputProcessor** 再加工：

```
Scheduler.update_from_output(scheduler_output, model_output)
    → EngineCoreOutputs   ***REMOVED*** 按 request_id 的列表，每项包含：
         - new_token_ids: list[int]   ***REMOVED*** 本步新生成的 token id
         - finish_reason, stop_reason ***REMOVED*** 是否结束、stop 原因
         - pooling_output 等（embedding 等非生成场景）

OutputProcessor.process_outputs(engine_core_outputs)
    for each EngineCoreOutput:
        Detokenizer.update(new_token_ids, ...)   ***REMOVED*** 增量 decode → 文本，stop 串检查
        make_request_output(...)
            → RequestOutput   ***REMOVED*** 含 outputs[].text, token_ids, logprobs, finish_reason
```
...
***REMOVED******REMOVED******REMOVED*** 0.3 小结
...
***REMOVED******REMOVED******REMOVED*** 0.4 模型端输出代码位置

**核心文件**：`vllm/v1/worker/gpu_model_runner.py`

***REMOVED******REMOVED******REMOVED******REMOVED*** 1. 模型 forward 与 logits 计算（约 2298-2331 行）

```python
***REMOVED*** 2298-2304: 模型 forward
...
***REMOVED*** 2306-2312: 提取 hidden_states
...
***REMOVED*** 2330-2331: 计算 logits
sample_hidden_states = hidden_states[logits_indices]
logits = self.model.compute_logits(sample_hidden_states)
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 2. 采样（约 2054-2092 行）
...
def _sample(
    ...
    if spec_decode_metadata is None:
        ...
        ***REMOVED*** speculative decoding 分支
        ...
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 3. 构造并返回 ModelRunnerOutput（约 2426-2445 行）

```python
output = ModelRunnerOutput(
    req_ids=req_ids_output_copy,
    req_id_to_index=req_id_to_index_output_copy,
    sampled_token_ids=valid_sampled_token_ids,  ***REMOVED*** list[list[int]]
    logprobs=logprobs_lists,                     ***REMOVED*** LogprobsLists 或 None
...
if not self.use_async_scheduling:
    ...
    return AsyncGPUModelRunnerOutput(...)  ***REMOVED*** 异步调度包装
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 4. 数据类定义

- **SamplerOutput**：`vllm/v1/outputs.py` 约 74-81 行
  ```python
  @dataclass
  class SamplerOutput:
      sampled_token_ids: torch.Tensor  ***REMOVED*** [num_reqs, max_num_generated_tokens]
...
- **ModelRunnerOutput**：`vllm/v1/outputs.py` 约 99-120 行
  ...
  class ModelRunnerOutput:
      ...
      sampled_token_ids: list[list[int]]  ***REMOVED*** num_reqs x num_generated_tokens
      logprobs: Optional[LogprobsLists]
      prompt_logprobs_dict: dict[str, ...]
      ...
  ```

***REMOVED******REMOVED******REMOVED******REMOVED*** 5. 完整流程（execute_model 方法，约 2231-2445 行）
...
def execute_model(self, scheduler_output, ...) -> ModelRunnerOutput:
    ...
    ***REMOVED*** 2. 模型 forward（2298-2304）
    model_output = self.model(...)
    hidden_states = model_output  ***REMOVED*** 或 model_output[0]
    
    ***REMOVED*** 3. 计算 logits（2330-2331）
    logits = self.model.compute_logits(hidden_states[logits_indices])
    
    ***REMOVED*** 4. 采样（2366-2367）
    sampler_output = self._sample(logits, spec_decode_metadata)
    
    ***REMOVED*** 5. 后处理：bookkeeping、整理数据（2404-2415）
    valid_sampled_token_ids, logprobs_lists, ... = self._bookkeeping_sync(...)
    
    ***REMOVED*** 6. 构造并返回 ModelRunnerOutput（2426-2435）
...
***REMOVED******REMOVED******REMOVED*** 0.5 模型端的流式实现：模型端本身不做流式

**重要结论**：**模型端本身不做流式**，它只是**每步返回一次结果**。流式是在**引擎层**通过**循环调用模型端**实现的。

***REMOVED******REMOVED******REMOVED******REMOVED*** 模型端：同步、每步返回
...
def execute_model(
    ...
    ...
) -> ModelRunnerOutput:  ***REMOVED*** 返回单个 ModelRunnerOutput，不是 Iterator/Generator
    ***REMOVED*** 1. 模型 forward
    model_output = self.model(...)
    
    ***REMOVED*** 2. 计算 logits、采样
    logits = self.model.compute_logits(...)
    sampler_output = self._sample(logits, ...)
    
    ***REMOVED*** 3. 返回本步的 ModelRunnerOutput
    return ModelRunnerOutput(
        sampled_token_ids=valid_sampled_token_ids,  ***REMOVED*** 本步的 token ids
        logprobs=logprobs_lists,
        ...
    )
```
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 引擎层：循环调用模型端，实现流式

流式是在**引擎层**实现的，通过**循环调用模型端**：

```python
***REMOVED*** vllm/v1/engine/core.py 约 272-291 行
def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
    """每步：调度、执行模型、更新 scheduler"""
    scheduler_output = self.scheduler.schedule()
    model_output = self.execute_model_with_error_logging(
        self.model_executor.execute_model,  ***REMOVED*** 调用模型端
        scheduler_output)
    engine_core_outputs = self.scheduler.update_from_output(
        scheduler_output, model_output)
    return engine_core_outputs, ...

***REMOVED*** 引擎的后台循环（run_engine_loop）会不断调用 step()
async def run_engine_loop(self):
    while True:
        has_requests = await self.engine_step()  ***REMOVED*** 内部调用 step()
        if has_requests:
            ***REMOVED*** 每步产生一批 RequestOutput，通过 AsyncStream 推给调用方
...
***REMOVED******REMOVED******REMOVED******REMOVED*** 流式粒度
...
***REMOVED******REMOVED*** 2. 核心组件

***REMOVED******REMOVED******REMOVED*** 2.1 AsyncStream（`vllm.engine.async_llm_engine`）
...
class AsyncStream:
    ...
    async def __anext__(self) -> RequestOutput:
        ...
        if isinstance(result, Exception):
            raise result
        return result   ***REMOVED*** 通常是 RequestOutput
```
...
***REMOVED******REMOVED******REMOVED*** 2.2 RequestTracker
...
***REMOVED******REMOVED******REMOVED*** 2.3 后台循环 `run_engine_loop`
...
***REMOVED******REMOVED******REMOVED*** 2.4 AsyncLLMEngine.generate
...
***REMOVED******REMOVED*** 3. 流式粒度：step 与 RequestOutput
...
```python
***REMOVED*** 例如 LlamaFactory vllm_engine 的 stream_chat
generated_text = ""
...
***REMOVED******REMOVED*** 4. 与 OpenAI HTTP 流式的关系
