            ...
            try:
                ...
                if use_single_model_only:
                    ...
                    try:
                        ...
                        # 使用 Kimi K2 和 Gemini 进行投票（推荐配置：Kimi 实体提取最强，Gemini 作为验证）
                        # 优先保留 Kimi 的所有结果，Gemini 作为补充
...
4. JSON 结构完整且有效"""
    ...
    def _create_novel_plan_progressive(
        ...
        plan = {
            ...
            "current_phase": 1,
            # 为了向后兼容，提供 chapter_outline（基于第一阶段）
    ...
    def _generate_overall_outline(
        ...
        Returns:
            整体大纲
        """
        num_phases = (target_chapters + 19) // 20  # 向上取整
...
"""
    ...
    def _generate_phase_outline(
        ...
        target_chapters = overall_outline.get('target_chapters', phase_number * phase_size)
        if end_chapter >= target_chapters - phase_size // 2:  # 最后阶段或接近最后
            is_final_phase = True
...
"""
    ...
    def _get_rhythm_adjusted_target_words(
        ...
        position_adjustments = {
            'opening': 0.95,    # 开头章节稍短，快速进入
            'development': 1.0,  # 发展章节保持标准
            'climax': 1.15,     # 高潮章节允许更长
            'ending': 0.90      # 结尾章节稍短，简洁收尾
        }
        ...
        if recent_rhythm_data and len(recent_rhythm_data) >= 2:
            ...
            if abs(avg_recent - base_target_words) / base_target_words < 0.05:
                ...
                if chapter_position == 'climax':
                    adjusted_words = int(adjusted_words * 1.10)  # 高潮章节更长
                elif chapter_position == 'ending':
                    adjusted_words = int(adjusted_words * 0.95)  # 结尾章节更短
        
        # 确保在合理范围内（基础字数的80%-120%）
...
"""
        ...
        # 初始生成策略：质量优先，字数控制通过prompt实现
        # max_new_tokens 始终不低于 2048，确保有足够的生成空间
        MIN_TOKEN_LIMIT = 2048
        
        # 计算基础token限制（确保至少2048，根据目标字数适当调整）
        # 目标2048字时，使用3072 token（1.5倍），确保有足够空间生成优质内容
        ...
        # 自适应截断策略：
        # 1. 如果超过上限3000字，截断到上限
        # 2. 如果超过目标字数的20%（目标2048 → 2458字），截断到目标+15%（2355字）
        #    这样可以更严格地控制字数，使其更接近目标
        strict_cutoff = int(target_words * 1.20)  # 目标+20%，超过此值需要截断
        ...
        # 检查字数是否过少（少于目标字数的50%），如果是则重试生成
        min_acceptable_words = int(target_words * 0.5)  # 最少接受目标字数的50%
