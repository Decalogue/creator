***REMOVED*** BookMemory：书籍专用记忆架构

***REMOVED******REMOVED*** 一、设计理念
...
***REMOVED******REMOVED*** 三、核心组件设计

***REMOVED******REMOVED******REMOVED*** 1. 书籍结构化表示（Book Structure）
...
class Book:
    ...
    metadata: dict
        ...
        - style: 风格
        - period: 时代背景
    chapters: List[Chapter]  ***REMOVED*** 章节列表
    characters: List[Character]  ***REMOVED*** 人物列表
    plotlines: List[Plotline]  ***REMOVED*** 情节线列表
    themes: List[Theme]  ***REMOVED*** 主题列表
    timeline: Timeline  ***REMOVED*** 时间线
...
class Chapter:
    ...
    title: str
    content: str
    summary: str  ***REMOVED*** 章节摘要
    key_events: List[Event]  ***REMOVED*** 关键事件
    characters_involved: List[str]  ***REMOVED*** 涉及的人物ID
    plotlines_involved: List[str]  ***REMOVED*** 涉及的情节线ID
    themes: List[str]  ***REMOVED*** 主题标签
    position_in_story: float  ***REMOVED*** 在故事中的位置（0.0-1.0）
...
class Character:
    ...
    book_id: str
    name: str
    aliases: List[str]  ***REMOVED*** 别名
    description: str  ***REMOVED*** 人物描述
    attributes: dict  ***REMOVED*** 属性
        - age: 年龄
        - gender: 性别
        - role: 角色（主角/配角/反派等）
        - personality: 性格特征
        - background: 背景
    relationships: List[Relationship]  ***REMOVED*** 人物关系
    appearances: List[str]  ***REMOVED*** 出现的章节ID
    character_arc: CharacterArc  ***REMOVED*** 人物弧线
    embedding: np.ndarray
...
class Relationship:
    ...
    character1_id: str
    character2_id: str
    relation_type: str  ***REMOVED*** 关系类型（朋友/敌人/恋人/父子等）
    strength: float  ***REMOVED*** 关系强度（0.0-1.0）
    description: str  ***REMOVED*** 关系描述
    timeline: List[RelationshipEvent]  ***REMOVED*** 关系发展时间线
...
class Plotline:
    ...
    name: str
    description: str
    type: str  ***REMOVED*** 情节类型（主线/支线/伏笔等）
    chapters: List[str]  ***REMOVED*** 涉及的章节ID
    key_events: List[Event]  ***REMOVED*** 关键事件
    resolution: str  ***REMOVED*** 结局/解决方式
    embedding: np.ndarray
...
class Event:
    ...
    chapter_id: str
    description: str
    timestamp: float  ***REMOVED*** 在故事中的时间位置
    characters_involved: List[str]
    plotlines_involved: List[str]
    importance: float  ***REMOVED*** 重要性评分
    causal_links: List[str]  ***REMOVED*** 因果关联的事件ID
...
class Theme:
    ...
    name: str
    description: str
    chapters: List[str]  ***REMOVED*** 相关章节
    characters: List[str]  ***REMOVED*** 相关人物
    embedding: np.ndarray
```

---

***REMOVED******REMOVED******REMOVED*** 2. 拆解层（Decomposition Layer）
...
***REMOVED******REMOVED******REMOVED*** 3. 存储层（Storage Layer）- 基于 CogMem + LightMem
...
class BookMemoryStorage:
    ...
    class FocusOfAttention:
        ...
        def __init__(self):
            ...
            self.context_chapters: List[Chapter] = []  ***REMOVED*** 前后章节上下文
    ...
    class DirectAccess:
        ...
        """
        def __init__(self):
            self.recent_books: Dict[str, Book] = {}  ***REMOVED*** 最近访问
            self.related_books: Dict[str, Book] = {}  ***REMOVED*** 相关书籍
            self.similar_books: Dict[str, Book] = {}  ***REMOVED*** 相似书籍
    ...
    class LongTermMemory:
        ...
        """
        def __init__(self):
            self.book_db: Database  ***REMOVED*** 书籍数据库
            self.vector_db: VectorDB  ***REMOVED*** 向量数据库
            self.graph_db: GraphDB  ***REMOVED*** 图数据库
...
***REMOVED******REMOVED******REMOVED*** 4. 关系层（Graph Layer）- 基于 A-Mem + Graph of Records
...
class BookMemoryGraph:
    ...
    class PlotlineGraph:
        ...
        def __init__(self):
            ...
            self.causal_edges: List[Tuple[str, str]] = []  ***REMOVED*** 因果关系
            self.temporal_edges: List[Tuple[str, str]] = []  ***REMOVED*** 时序关系
    ...
    class ThemeGraph:
        ...
        def __init__(self):
            ...
            self.association_edges: List[Tuple[str, str, float]] = []  ***REMOVED*** 主题关联
...
***REMOVED******REMOVED******REMOVED*** 5. 检索层（Retrieval Layer）- 基于 Every Token Counts
...
class BookMemoryRetrieval:
    ...
    def retrieve_similar_books(self, book: Book, top_k: int = 5):
        """检索相似书籍"""
        ***REMOVED*** 基于：类型、主题、风格、人物、情节
        pass
```

---

***REMOVED******REMOVED******REMOVED*** 6. 创作层（Creation Layer）
...
***REMOVED******REMOVED******REMOVED*** 7. 分享层（Sharing Layer）
...
***REMOVED******REMOVED*** 四、完整工作流程

***REMOVED******REMOVED******REMOVED*** 场景 1：书籍拆解与记忆
...
***REMOVED******REMOVED******REMOVED*** 场景 2：创作支持
...
***REMOVED******REMOVED******REMOVED*** 场景 3：内容分享
...
***REMOVED******REMOVED*** 五、关键技术实现

***REMOVED******REMOVED******REMOVED*** 1. 人物关系识别
...
***REMOVED******REMOVED******REMOVED*** 2. 情节线识别
...
***REMOVED******REMOVED******REMOVED*** 3. 一致性验证
...
***REMOVED******REMOVED*** 六、技术栈

***REMOVED******REMOVED******REMOVED*** 存储
...
***REMOVED******REMOVED******REMOVED*** LLM 模型配置（多模型策略）

***REMOVED******REMOVED******REMOVED******REMOVED*** 默认模型配置
...
class ModelConfig:
    ...
    ***REMOVED*** 主模型（默认用于核心创作任务）
    DEFAULT_PRIMARY_MODEL = "gpt-4o"  ***REMOVED*** 或 "claude-3.5-sonnet"
    
    ***REMOVED*** 任务专用模型配置
    MODELS = {
        ***REMOVED*** 书籍拆解任务（需要深度理解）
        "decomposition": {
            "primary": "gpt-4o",  ***REMOVED*** 或 "claude-3.5-sonnet"
        ...
        ***REMOVED*** 摘要生成（快速任务）
        "summary_generation": {
            "primary": "gpt-4-turbo",  ***REMOVED*** 或 "claude-3-haiku"
        ...
        ***REMOVED*** 播客脚本生成
        "podcast_script": {
            "primary": "claude-3.5-sonnet",  ***REMOVED*** 对话风格更好
            "fallback": "gpt-4o",
            "reason": "需要自然对话风格"
        },
    ...
    ***REMOVED*** 多模型集成策略（可选）
    ENSEMBLE_CONFIG = {
        "enabled": False,  ***REMOVED*** 默认关闭，需要高质量时开启
        "tasks": ["chapter_generation"],  ***REMOVED*** 仅对关键任务使用
        "models": ["gpt-4o", "claude-3.5-sonnet"],
        "strategy": "vote"  ***REMOVED*** 或 "weighted_average"
    }
```

***REMOVED******REMOVED******REMOVED******REMOVED*** 模型选择策略
...
***REMOVED******REMOVED*** 八、实现优先级

***REMOVED******REMOVED******REMOVED*** Phase 1: 核心拆解与存储（MVP）
1. ✅ 书籍结构化拆解（章节、人物、情节）
2. ✅ 基础存储（LTM）
3. ✅ 简单关系图（人物关系）

***REMOVED******REMOVED******REMOVED*** Phase 2: 检索与创作
4. ✅ 智能检索系统
5. ✅ 大纲生成
6. ✅ 章节生成
7. ✅ 一致性验证

***REMOVED******REMOVED******REMOVED*** Phase 3: 高级功能
8. ✅ 复杂关系网络（情节关系、主题关系）
9. ✅ 跨书籍检索
10. ✅ 多样化分享（播客、分析报告）
...
***REMOVED******REMOVED*** 九、示例用例

***REMOVED******REMOVED******REMOVED*** 用例 1：拆解《三体》

```python
book = decomposer.decompose("三体.txt")

***REMOVED*** 结果：
***REMOVED*** - 3个主要人物：叶文洁、汪淼、史强
***REMOVED*** - 多条情节线：三体入侵、科学探索、人性思考
***REMOVED*** - 多个主题：科学、人性、文明、生存
***REMOVED*** - 复杂人物关系网络
***REMOVED*** - 完整时间线
```

***REMOVED******REMOVED******REMOVED*** 用例 2：创作科幻小说
...
***REMOVED******REMOVED******REMOVED*** 用例 3：生成播客脚本
